...
"""
def parse_build_401_binary_c2(data):
    controllers = []
    c2_offset = 0
    c2_data = data
...
def decrypt_build_401_data(data):
    if not data:
        return
    ...
    for rsrc in pe.DIRECTORY_ENTRY_RESOURCE.entries:
        for entry in rsrc.directory.entries:
            if entry.name is not None:
                #print("id:{}".format(entry.name.__str__()))
                ...
                if entry.name.__str__() == '307':
                        ...
                        for entry in rsrc.directory.entries:
                            if entry.name is not None:
                                ...
                                if entry.name.__str__() == '308':
                                    ...
                                    #print("qbot_config:{}".format(config))
                                    print("other", { "Core DLL Build": parse_build(pe2).decode("utf-8") })
                    ...
                    config = parse_config(dec_bytes)
                    #print("qbot_config:{}".format(config))
                    print("other", { "Core DLL Build": parse_build(pe)})
                    ...
                    dec_bytes = decrypt_build_401_data(res_data)
                    controllers = parse_build_401_binary_c2(dec_bytes)
                    for controller in controllers:
                        print("address", controller)
